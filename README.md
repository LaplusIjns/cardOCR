...
圖片 / PDF
  ...
  -> 核心欄位仍空白：完整名片頁面 -> OpenAI 最終視覺確認（最多一次）
...
OCR block 不是業務欄位。例如 PaddleOCR 分別回傳同一行的 `F` 與 `03-12345678` 時，Java 會先依座標重建為 `F | 03-12345678`，再由規則引擎判定為 Fax。若姓名因排版字距被辨識成 `王 | 小 | 明` 或 `王 | O | O`，版面層會保留原始 block，同時建立帶來源座標的緊密候選；規則引擎只將其路由為疑似姓名，最後由文字上下文與優先局部裁圖確認。若 OCR 只偵測到單一常見姓氏，也會擴大裁圖以涵蓋旁邊未形成文字 block 的字形。第一次流程結束後，`name`、`companyName`、`jobTitle` 任一仍為空白時，系統會再以完整名片頁面做一次最終視覺確認。
...
OpenAI key 在文件存在語意歧義，或核心欄位需要最終視覺確認時是必要的。OpenAI 請求使用 Responses API 的 JSON Schema Structured Output；Java 會再次驗證欄位格式。最終確認只合併原本空白的 `name`、`companyName`、`jobTitle`，模型回傳的其他欄位一律忽略。
